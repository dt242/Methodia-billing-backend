...
public class PdfGenerationService {
    ...
    public byte[] generateInvoicePdf(Invoice invoice) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            try (InputStream regularFontStream = getClass().getResourceAsStream("/fonts/arial.ttf");
                 InputStream boldFontStream = getClass().getResourceAsStream("/fonts/arialbd.ttf")) {

                if (regularFontStream == null || boldFontStream == null) {
                    throw new RuntimeException("Шрифтовете не бяха намерени в /fonts/ директорията.");
                }

                byte[] arialBytes = regularFontStream.readAllBytes();
                byte[] arialBoldBytes = boldFontStream.readAllBytes();

                BaseFont baseFont = BaseFont.createFont("arial.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, true, arialBytes, null);
                BaseFont baseFontBold = BaseFont.createFont("arialbd.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, true, arialBoldBytes, null);
